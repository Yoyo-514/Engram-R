...
import { WorkflowEngine, type WorkflowDefinition } from '@/modules/workflow/core/WorkflowEngine';
import { type IStep } from '@/modules/workflow/core/Step';
...
describe('WorkflowEngine Retry Mechanism', () => {
  ...
  it('should abort retries if context.signal is cancelled during wait', async () => {
    ...
    await expect(promise).rejects.toThrow('UserCancelled'); // 取消应向上层显式传播为统一的取消错误。
    expect(executeMock).toHaveBeenCalledTimes(1);
  });

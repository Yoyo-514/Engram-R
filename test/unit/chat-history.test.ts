...
describe('ChatHistoryHelper Unit Tests', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (global as any).window.TavernHelper.formatAsTavernRegexedString = vi.fn(
  ...
  describe('getChatHistory', () => {
    ...
    it('should route Tavern regex by message role', () => {
      ...
      expect(
        (global as any).window.TavernHelper.formatAsTavernRegexedString
      ).toHaveBeenNthCalledWith(1, 'Msg 1', 'user_input', 'prompt');
      expect(
        (global as any).window.TavernHelper.formatAsTavernRegexedString
      ).toHaveBeenNthCalledWith(2, 'Msg 2', 'ai_output', 'prompt');
    });
  });

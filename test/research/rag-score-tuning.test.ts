...
describe('RAG Score Tuning - BrainRecallCache Bug Reproduction', () => {
  ...
  it('should handle high-pressure scenario with 40 candidates', () => {
    ...
    console.log(
      `[Test] High Pressure: Top ID=${results[0].id}, Score=${results[0].finalScore.toFixed(3)}`
    );
    console.log(
      `[Test] High Pressure: 10th ID=${results[9].id}, Score=${results[9].finalScore.toFixed(3)}`
    );
  });
});

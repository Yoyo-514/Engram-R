...
describe('CI Review 5: Logic Fixes', () => {
        ...
        patches: [
          {
            op: 'add',
            path: '/entities/Alina',
            value: { type: 'char', profile: { mood: 'happy' } },
          },
        ],
      },
      config: { dryRun: false },
  ...
  it('should handle case-insensitive duplication (SaveEntity P0)', async () => {
    ...
    const context: any = {
      input: {},
      parsedData: {
        patches: [{ op: 'add', path: '/entities/alina', value: { type: 'char' } }],
      },
      config: { dryRun: false },
    };
  ...
  it('should merge unified patch when root add hits existing alias', async () => {
    const step = new SaveEntity();
    const context: any = {
      input: {
        _rawExistingEntities: [
          {
            id: '1',
            name: 'Alina',
            aliases: ['Captain Alina'],
            profile: { age: 18 },
            type: 'char',
          },
        ],
      ...
      parsedData: {
        patches: [
          {
            op: 'add',
            path: '/entities/Captain%20Alina',
            value: { type: 'char', profile: { mood: 'focused' } },
          },
        ],
      },
      config: { dryRun: false },
    ...
    expect(updateEntityMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        name: 'Alina',
        profile: expect.objectContaining({ mood: 'focused' }),
      })
    );
  });
});

...
describe('BrainRecallCache', () => {
  ...
  describe('Decay Bomb (上下文切换)', () => {
    it('全新候选（零重叠）应触发 Decay Bomb', () => {
      // 先建立旧记忆
      cache.nextRound();
      cache.process([makeCandidate('evt_old_1', 0.8, 0.7), makeCandidate('evt_old_2', 0.7, 0.6)]);
      ...
      cache.nextRound();
      cache.process([makeCandidate('evt_new_1', 0.8, 0.7), makeCandidate('evt_new_2', 0.8, 0.7)]);
  ...
  describe('容量管理', () => {
    it('shortTermMemory 不应超过 shortTermLimit', () => {
      ...
      for (let round = 0; round < 5; round++) {
        cache.nextRound();
        cache.process(
          makeCandidates(5, 0.8 - round * 0.05).map((c) => ({
            ...c,
            id: `r${round}_${c.id}`,
          }))
        );
    ...
    it('低分记忆应优先被淘汰', () => {
      ...
      cache.nextRound();
      cache.process([
        makeCandidate('high', 0.95, 0.9),
        makeCandidate('mid', 0.7, 0.6),
        makeCandidate('low', 0.3, 0.2),
      ]);

      // 再加更多，触发淘汰
      cache.nextRound();
      cache.process([
        makeCandidate('new_1', 0.85, 0.8),
        makeCandidate('new_2', 0.8, 0.75),
        makeCandidate('new_3', 0.75, 0.7),
      ]);
  ...
  describe('厌倦惩罚 (Boredom Penalty)', () => {
    it('连续进入工作记忆超过阈值应被惩罚', () => {
      ...
      for (let i = 0; i < 6; i++) {
        cache.nextRound();
        cache.process([makeCandidate('evt_001', 0.8, 0.7), makeCandidate('evt_002', 0.75, 0.65)]);
      }
  ...
  describe('多轮模拟 (Integration)', () => {
    it('完整的 5 轮对话模拟', () => {
      ...
      working = cache.process([
        makeCandidate('tavern_meet', 0.85, 0.8),
      ...
      cache.nextRound();
      working = cache.process([
        makeCandidate('old_quest', 0.85, 0.8),
        makeCandidate('quest_reward', 0.75, 0.7),
      ]);
      results.push({
        round: 3,
        working: working.map((s) => s.id),
      ...
      working = cache.process([
        ...
        makeCandidate('forest_monster', 0.85, 0.8),
      ...
      working = cache.process([
        ...
        makeCandidate('forest_boss', 0.85, 0.8),
      ]);
      results.push({
        round: 5,
        working: working.map((s) => s.id),

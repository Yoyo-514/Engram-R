...
export class HistoryTask implements IBatchTaskHandler {
  ...
  async estimate(): Promise<BatchTask[]> {
    ...
    const entityInterval = targetTypes.has('summary')
      ? summaryInterval
      : entityConfig.floorInterval || 50;

    const summaryTasksCount = targetTypes.has('summary')
      ? Math.ceil(floorRange / summaryInterval)
      : 0;
    const entityTasksCount =
      targetTypes.has('entity') && entityConfig.enabled
        ? Math.ceil(floorRange / entityInterval)
        : 0;
    const trimThreshold = trimConfig.countLimit || 5;
    const trimTasksCount =
      targetTypes.has('trim') && trimConfig.enabled
        ? Math.floor(summaryTasksCount / trimThreshold)
        : 0;
    // 粗略估算 embed 次数
  ...
  private async *executeSummary(
    ...
    while (processedFloors < totalToProcess) {
      ...
      const currentSliceEnd = Math.min(
        task.floorRange.end,
        currentSliceStart + summaryInterval - 1
      );
      ...
      updateProgress(
        taskIndex,
        Math.min(task.progress.total, Math.ceil(processedFloors / summaryInterval))
      );
    }
  }
    ...
    while (processedFloors < task.floorRange.end - task.floorRange.start) {
      ...
      try {
        ...
        if (res && !res.success) {
          Logger.warn(
            LogModule.BATCH,
            `Entity extract failed for range ${currentSliceStart}-${currentSliceEnd}`,
            { error: res.error }
          );
        }
      } catch (err: any) {
      ...
      updateProgress(
        taskIndex,
        Math.min(task.progress.total, Math.ceil(processedFloors / entityInterval))
      );
    }
  }
  ...
  private async *executeEmbed(
    ...
    const res = await embeddingService.embedUnprocessedEvents((current, total) => {
      ...
      updateProgress(
        taskIndex,
        Math.min(task.progress.total, Math.ceil((current / total) * task.progress.total))
      );
    });

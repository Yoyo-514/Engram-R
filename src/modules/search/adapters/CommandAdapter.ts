...
export class CommandAdapter implements SearchAdapter {
  async search(query: string): Promise<SearchResult[]> {
    ...
    const navResults: SearchResult[] = NAV_ITEMS.map((item: NavItem) => ({
      ...
      (item) => item.title.toLowerCase().includes(lowerQuery) || item.id.includes(lowerQuery)
    );
    ...
    const manualResults: SearchResult[] = COMMANDS.filter(
      (cmd) =>
    ...
    const themeResults = this.getThemeCommands().filter(
      (cmd) =>

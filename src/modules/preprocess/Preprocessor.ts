...
import { createPreprocessWorkflow, runWorkflow, StopGeneration } from '@/modules/workflow';
import type { JobContext } from '@/types/job_context';
import type { AgenticRecall, PreprocessConfig, PreprocessResult } from '@/types/preprocess';
...
class Preprocessor {
   ...
   * @returns 预处理结果
   */
  async process(userInput: string): Promise<PreprocessResult> {
    ...
    const runningToast = notificationService.running('预处理中...', 'Engram', () => {
      ...
      cancelSignal.reason = 'preprocess_cancelled';
    ...
    try {
      // Lazy import
      const context = await runWorkflow(createPreprocessWorkflow(), {
        trigger: 'auto',
        signal: cancelSignal,
        config: {
      ...
      let agenticRecalls: AgenticRecall[] | undefined;
  ...
  getConfig(): PreprocessConfig {
    ...
    return config || DEFAULT_PREPROCESS_CONFIG;
  }

  /**
   * 保存预处理配置
   */
  saveConfig(config: PreprocessConfig): void {
    Logger.debug(LogModule.PREPROCESS, '保存配置', config);
    set('preprocessConfig', config);
  }

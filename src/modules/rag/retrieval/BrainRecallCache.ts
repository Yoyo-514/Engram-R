...
export class BrainRecallCache {
  ...
  process(candidates: RecallCandidate[]): MemorySlot[] {
    ...
    Logger.debug(
      LogModule.RAG_CACHE,
      `Evicted ${sizeBeforeEvict - this.shortTermMemory.size} items. Current Size: ${this.shortTermMemory.size}`
    );
  ...
  getShortTermSnapshot(): MemorySlot[] {
    return Array.from(this.shortTermMemory.values()).sort((a, b) => b.finalScore - a.finalScore);
  }

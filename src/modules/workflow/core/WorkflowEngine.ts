...
import { type IStep, type StepResult } from './Step';
...
export class WorkflowEngine {
   ...
   * 执行带有重试机制的步骤
   */
  private static async executeWithRetry(step: IStep, context: JobContext): Promise<StepResult> {
          ...
          if (result) {
            ...
            if (result.action === 'jump') {
              ...
              if (context.metadata.jumpCount > 50) {
                throw new Error(
                  `Workflow detected infinite loop: jumped 50 times. Last jump: ${step.name} -> ${result.targetStep}`
                );
              }

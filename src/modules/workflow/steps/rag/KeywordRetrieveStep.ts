...
export class KeywordRetrieveStep implements IStep {
  ...
  async execute(context: JobContext): Promise<void> {
    ...
    if (recallConfig?.enableEntityKeyword !== false) {
      ...
      if (matchedIndex.length > 0) {
        ...
        Logger.debug(
          ...
          `命中了 ${hitEntities.length} 个实体(TopK=${entityTopK}): ${hitEntities.map((e) => e.name).join(', ')}`
        );
      }
    } else {
      ...
      for (const targetName of Object.keys(relations)) {
        ...
        if (targetEntity) {
          ...
          if (hopScore > currentScore) {
            ...
            Logger.debug(
              LogModule.RAG_INJECT,
              `[多跳联想] 由 ${seedEntity.name} 联想到了 ${targetEntity.name} (${hopScore.toFixed(2)})`
            );
          }
        }
      }
    ...
    const keywordEntityIds = Array.from(keywordEntityMap.entries()).map(([id, score]) => ({
      id,
      score,
    }));
    ...
    Logger.debug(
      LogModule.RAG_INJECT,
      `关键词扫描完成，耗时 ${context.data.keywordRetrieveTime}ms。命中实体: ${hitEntities.length} 个，命中事件: ${hitEvents.length} 个`
    );
  }
}

...
export class SaveEntity implements IStep {
    ...
    private async processUnifiedPatches(
        ...
        for (const [entityName, entityPatches] of patchesByEntity) {
            ...
            const addRootPatch = entityPatches.find(p => p.op === 'add' && this.isEntityRootPath(p.path, entityName));
    ...
    /** 消除别名歧义匹配 */
    private resolveEntityIdentity(entityName: string, entityIndexes: EntityIndexes): EntityNode | undefined {
        // 1. 精确匹配名称优先
    ...
    private convertRootAddToPatches(entityName: string, value: any, entityPatches: any[]) {
        if (value && typeof value === 'object') {
            if (value.profile) entityPatches.push({ op: 'add', path: this.getEntityPath(entityName, '/profile'), value: value.profile });
            if (value.type) entityPatches.push({ op: 'replace', path: this.getEntityPath(entityName, '/type'), value: value.type });
            if (value.aliases) entityPatches.push({ op: 'add', path: this.getEntityPath(entityName, '/aliases'), value: value.aliases });
        }
    }
    ...
    private buildRelativePatches(entityName: string, entityPatches: any[], targetDoc: any): any[] {
        ...
        for (const p of entityPatches) {
            const isRoot = this.isEntityRootPath(p.path, entityName);
            ...
            if (!isRoot) {
                let relPath = this.stripEntityPrefix(p.path, entityName);
    ...
    private async processLegacyFormat(
        ...
        if (data.entities) {
            for (const extracted of data.entities) {
                const exists = entityIndexes.byName.get(extracted.name) || this.pickAliasMatch(extracted.name, entityIndexes.byAlias);
                if (exists) continue;
                ...
                if (!isDryRun) {
                    ...
                    entity.id = `temp-${Date.now()}`;
                    newEntities.push(entity);
                    this.indexEntity(entityIndexes, entity as EntityNode);
                }
            }
        }
    ...
    private pickAliasMatch(entityName: string, aliasIndex: Map<string, EntityNode[]>): EntityNode | undefined {
        ...
        if (aliasMatches.length > 1) {
            Logger.warn('SaveEntity', `⚠️ Alias conflict detected for "${entityName}". Multiple entities share this alias. Falling back to first match to avoid crash, but data overwrite may occur.`, {
    ...
    private findCaseInsensitiveConflict(entityName: string, entityIndexes: EntityIndexes): EntityNode | undefined {
        const normalizedName = entityName.toLowerCase();
        return entityIndexes.byLowerName.get(normalizedName)

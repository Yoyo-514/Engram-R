...
function isReviewPayload(value: unknown): value is ReviewPayload {
  ...
  return (
    ...
    (value.agenticRecalls === undefined || Array.isArray(value.agenticRecalls)) &&
    (value.newEntities === undefined || Array.isArray(value.newEntities)) &&
    (value.updatedEntities === undefined || Array.isArray(value.updatedEntities))
  );
}
    ...
    try {
      ...
      if (isReviewPayload(result.data)) {
        ...
        if (context.extractedTags) {
          if (reviewData.query !== undefined) {
            context.extractedTags.query = reviewData.query;
            Logger.debug('UserReview', 'Query 已更新', { query: reviewData.query });
          }

          if (reviewData.agenticRecalls !== undefined) {
            context.extractedTags.recall_decision = JSON.stringify({
              recalls: reviewData.agenticRecalls,
            });
            Logger.debug('UserReview', 'Agentic 召回决策已更新', {
              count: reviewData.agenticRecalls.length,
            });
          }
        }

        // 如果是实体数据，更新 parsedData 和 output
        if (reviewData.newEntities || reviewData.updatedEntities) {
          context.parsedData = reviewData;
          context.output = reviewData;
        } else {
          context.output = result.content;
        }
      } else {

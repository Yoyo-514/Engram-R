...
class NotificationService {
  ...
  private getToastr(): Toastr | null {
    const hostWindow = window as Window & { toastr?: Toastr };
    return hostWindow.toastr ?? null;
  }
  ...
  public success(
    ...
    const finalOnClick = action?.onClick
      ? action.onClick
      : actionGoto
        ? () => this.navigate(actionGoto)
        : undefined;
    ...
    if (toastr) {
      ...
      console.info(`[Engram] SUCCESS: ${title} - ${message}`);
    }
  ...
  public info(message: string, title: string = 'Engram', options: NotificationOptions = {}): void {
    ...
    if (toastr) {
      ...
      console.info(`[Engram] INFO: ${title} - ${message}`);
    }
  ...
  public error(message: string, title: string = 'Engram', options: NotificationOptions = {}): void {
    ...
    const defaultAction: NonNullable<NotificationOptions['action']> = options.action ?? {
      goto: 'devlog',
    };
    const defaultGoto = defaultAction.goto;
    const clickAction = defaultAction.onClick
      ? defaultAction.onClick
      : defaultGoto
        ? () => this.navigate(defaultGoto)
        : undefined;
    const finalOnClick = this.buildClickHandler(clickAction);
  ...
  public running(message: string, title: string = 'Engram', onCancel?: () => void): JQuery | null {
    ...
    if (toastr) {
      ...
      console.info(`[Engram] RUNNING: ${title} - ${message}`);
      return null;
    }
  }

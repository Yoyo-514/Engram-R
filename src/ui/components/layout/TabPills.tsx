...
export const TabPills: FC<TabPillsProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="flex overflow-x-auto gap-2 pb-1 no-scrollbar">
        {tabs.map((tab) => (
          ...
          >
            {tab.icon && (
              <span
                className={`w-4 h-4 ${enableAnimations ? 'transition-transform duration-[var(--duration-fast)] group-hover:scale-110' : ''}`}
              >
                {tab.icon}
              </span>
            )}
      ...
      {/* 右侧操作区域 */}
      {actions && <div className="flex items-center gap-2 pb-1 shrink-0">{actions}</div>}
    </div>
  );
};

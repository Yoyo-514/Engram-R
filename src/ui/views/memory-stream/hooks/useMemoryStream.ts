...
export function useMemoryStream(initialTab: ViewTab = 'list') {
  ...
  const handleToggleEventLock = useCallback(
    async (id: string, isLocked: boolean) => {
      try {
  ...
  const handleToggleEventArchive = useCallback(
    async (id: string, isArchived: boolean) => {
      try {
  ...
  const handleBatchSave = useCallback(async () => {
    ...
    try {
      const eventUpdates = Array.from(pendingChanges.entries()).map(([id, updates]) => ({
        id,
        updates,
      }));
      const entityUpdates = Array.from(pendingEntityChanges.entries()).map(([id, updates]) => ({
        id,
        updates,
      }));
  ...
  const handleReembedAll = useCallback(async () => {
    ...
    try {
      ...
      await embeddingService.reembedAllEvents((current, total, errors) => {
        console.info(`[MemoryStream] Reembedding: ${current}/${total}, errors: ${errors}`);
      });
      ...
      notificationService.error(
        `重嵌失败: ${error instanceof Error ? error.message : '未知错误'}`,
        'MemoryStream'
      );
    } finally {
  ...
  const handleCreateEvent = useCallback(async () => {
    try {
      ...
      notificationService.error(
        `创建事件失败: ${error instanceof Error ? error.message : '未知错误'}`,
        'MemoryStream'
      );
    }
  }, [store, loadEvents]);
  ...
  const handleCreateEntity = useCallback(async () => {
    try {
      ...
      notificationService.error(
        `创建实体失败: ${error instanceof Error ? error.message : '未知错误'}`,
        'MemoryStream'
      );
    }
  }, [store, loadEntities]);
  ...
  const handleOpenImportModal = useCallback(async () => {
    if (hasChanges) {
      if (
        !confirm('您有未保存的编辑内容，导入历史记忆库并合并可能覆盖您刚刚的修改，确定要继续吗？')
      ) {
        return;
      }
    }

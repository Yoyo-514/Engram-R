...
export const PromptTemplateForm: FC<PromptTemplateFormProps> = ({
  template,
  llmPresets,
  defaultPresetId,
  onChange,
  ...
  const modelSourceOptions = [
    {
      ...
      label:
        '跟随全局选中预设' +
        (defaultPresetId
          ? ` (${llmPresets.find((p) => p.id === defaultPresetId)?.name || defaultPresetId})`
          : ''),
    },
  ...
  // Group macros
  const groupedMacros = AVAILABLE_MACROS.reduce(
    (acc, macro) => {
      ...
      <FormSection title="基本信息">
        ...
        {template.category === 'preprocessing' && (
          <SelectField
            ...
            onChange={(value) =>
              updateTemplate({ injectionMode: value as 'replace' | 'append' | 'prepend' })
            }
            options={[

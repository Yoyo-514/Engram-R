...
export interface EventState {
  ...
  saveEvents: (
    events: (Omit<EventNode, 'id' | 'timestamp'> & { timestamp?: number })[]
  ) => Promise<EventNode[]>;
...
export const createEventSlice: StateCreator<any, [], [], EventState> = (set, _get) => ({
  ...
  archiveEvents: async (eventIds: string[]) => {
    ...
    try {
      ...
      console.info(`[MemoryStore] Archived ${eventIds.length} events`);
    } catch (e) {
  ...
  markEventsAsEmbedded: async (eventIds: string[]) => {
    ...
    try {
      ...
      console.info(`[MemoryStore] Marked ${eventIds.length} events as embedded`);
    } catch (e) {
  ...
  deleteEvents: async (eventIds: string[]) => {
    ...
    try {
      ...
      console.info(`[MemoryStore] Deleted ${eventIds.length} events`);
    } catch (e) {
  ...
  updateEvents: async (updatesList) => {
    ...
    try {
      ...
      console.info(`[MemoryStore] Batch updated ${updatesList.length} events`);
    } catch (e) {

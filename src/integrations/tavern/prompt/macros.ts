...
declare global {
  interface Window {
    EjsTemplate?: {
      ...
      evalTemplate: (content: string, context: Record<string, unknown>) => Promise<string>;
    };
    Mvu?: {
...
export class MacroService {
      ...
      if (eventSource) {
        ...
        eventSource.on('settings_updated', () => {
          this.refreshCache().catch((e) =>
            Logger.warn('MacroService', '设置更新后刷新缓存失败', e)
          );
        });
      }
    } catch (e) {
  ...
  static async refreshCache(recalledIds?: string[]): Promise<void> {
    await Promise.all([this.refreshEngramCache(recalledIds), this.refreshWorldbookCache()]);
   ...
   */
  private static registerMacro(name: string, handler: () => string, _description: string) {
    const context = getSTContext();

 ...
 * 查找已存在的 Engram 全局世界书
 */
export function findExistingWorldbook(): string | null {
  try {
    ...
    if (globalBooks.includes(ENGRAM_GLOBAL_WORLDBOOK_NAME)) {
      return ENGRAM_GLOBAL_WORLDBOOK_NAME;
    }
 ...
 * 获取或创建 Engram 全局世界书
 */
export async function getOrCreateWorldbook(): Promise<string | null> {
  try {
    const existing = findExistingWorldbook();
    if (existing) {
      return existing;
    }
    ...
    const worldbookName = ENGRAM_GLOBAL_WORLDBOOK_NAME;
 ...
 */
export function getScopes(): { global: string[]; chat: string[]; installed: string[] } {
  const helper = getTavernHelper();
  ...
  if (helper?.getCharWorldbookNames) {
    ...
    const hasCharacter =
      stContext && stContext.characterId !== undefined && stContext.characterId !== null;

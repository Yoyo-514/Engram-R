...
function isWorldbookFeatureEnabled(config: any): boolean {
...
function getWorldbooksForContextScan(filterState: Awaited<ReturnType<typeof loadFilteringState>>): string[] {
    const {
        config,
        globalWorldbooks,
        chatWorldbooks,
...
function resolveScanMessages(chatMessages?: string[], options?: { floorRange?: [number, number] }): string[] {
...
export class WorldbookScannerService {
    static async scanWorldbook(worldbookName: string, contextText: string, options?: { forceInclude?: boolean }): Promise<string> {
        ...
        if (!isWorldbookFeatureEnabled(config)) {
            Logger.debug(MODULE, 'Worldbook feature disabled, skipping scan', { worldbook: worldbookName });
            return '';
        }
        ...
        for (const entry of entries) {
            ...
            const matched = entry.keys.some((key: string) => key && lowerContext.includes(key.toLowerCase()));
            if (matched) {
                activeEntries.push(entry);
            }
        }

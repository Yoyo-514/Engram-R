...
export type TavernEventName = TavernEventTypeValue | (string & {});
...
export class EventBus {
   ...
   * @returns 取消订阅函数
   */
  static on(event: TavernEventName, callback: EventCallback): Unsubscribe {
   ...
   * @param callback 回调函数
   */
  static once(event: TavernEventName, callback: EventCallback): void {
    const eventSource = getEventSource();
   ...
   * @param callback 回调函数
   */
  static off(event: TavernEventName, callback: EventCallback): void {
    const eventSource = getEventSource();
   ...
   * @param args 参数
   */
  static async emit(event: TavernEventName, ...args: unknown[]): Promise<any> {
    const eventSource = getEventSource();

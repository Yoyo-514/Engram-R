...
function extractJsonBlock(text: string): string | null {
  ...
  // 比较哪个出现得更早且有效
  const foundObject =
    firstOpenBrace !== -1 && lastCloseBrace !== -1 && lastCloseBrace > firstOpenBrace;
  const foundArray =
    firstOpenBracket !== -1 && lastCloseBracket !== -1 && lastCloseBracket > firstOpenBracket;
...
export function parseJson<T = unknown>(input: string): T | null {
  // 1. 提取 JSON 块
  let jsonString = extractJsonBlock(input);
  ...
  if (jsonString.trim().startsWith('[')) {
    console.info('[RobustJsonParser] 检测到数组格式，尝试自动封装为 { events: [] }');
  ...
  try {
    ...
    // 首次解析失败：尝试简单的自动修复
    const repaired = simpleRepair(jsonString);
    try {

...
export class UpdateService {
  ...
  static async getExtensionRuntimeInfo(): Promise<ExtensionRuntimeInfo | null> {
    ...
    try {
      ...
      if (!response.ok) {
        console.debug(
          "[Engram] discover API 不可用，使用默认扩展标识",
          response.status,
        );
      } else {
        const extensions: DiscoveredExtension[] = await response.json();
        const found = extensions.find((ext) =>
          matchesExtensionName(
            extractExtensionLeaf(ext.name),
  ...
  static async hasUpdate(): Promise<boolean> {
    ...
    if (
      latestHash &&
      localRealHash !== "unknown" &&
      latestHash !== localRealHash
    ) {
  ...
  static async getChangelog(): Promise<string | null> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          "[Engram] UpdateService: 获取更新日志失败",
          response.status,
        );
        notificationService.warning(
          `获取更新日志失败: ${response.status}`,
          "更新检测",
        );
        return null;
      }
